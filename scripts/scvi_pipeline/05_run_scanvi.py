...
def main():
    ...
    prob_matrix = scanvi_model.predict(soft=True)
    if hasattr(prob_matrix, "to_numpy"):
        label_categories = list(prob_matrix.columns)
        prob_matrix = prob_matrix.to_numpy()
    else:
    ...
    adata.uns["scanvi_probability_labels"] = label_categories
